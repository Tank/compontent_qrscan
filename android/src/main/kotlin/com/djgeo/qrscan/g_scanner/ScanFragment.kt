...
import com.djgeo.qrscan.R
import com.djgeo.qrscan.g_scanner.QrCodeScannerActivity.Companion.REQUEST_CAMERA
...
class ScanFragment : Fragment(), ScanInteractorImpl.ScanCallbackInterface {
    ...
    companion object {
        ...
        fun newInstance(title: String, desc:String ,hasFlashLight: Boolean, toolBarColor: Int, titleColor: Int,
                        qrCornerColor: Int = 0, qrScanColor: Int = 0, scanAreaScale: Float): ScanFragment {

            val args = Bundle()
            args.putString(QrCodeScannerActivity.TITLE, title)
            args.putString(QrCodeScannerActivity.DESC, desc)
    ...
    private fun handleBundleData() {
        ...
        if (args != null && context != null) {
            ...
            val desc = args.getString(QrCodeScannerActivity.DESC,"")
            mTvDesc?.text = desc
          ...
          //  mFlashlightBtn?.visibility = if (hasFlashLight) View.VISIBLE else View.GONE
            mFlashlightBtn?.visibility = View.GONE
            ...
            if (titleColor != 0) {
                val drawable = ContextCompat.getDrawable(context!!, R.drawable.icon_return)
                drawable?.let {
